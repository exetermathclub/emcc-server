<?php
	...
	if ($_POST["account_update"])
	{
		...
		if (!$acc_error)
		{
			$email = "tamilsakthi@yahoo.com"; // mysql_real_escape_string($_SESSION["email"]);
	...
	$query11 =  "SELECT COUNT(*) AS COUNT FROM `emc2_tscores_2011` AS t1 " .
		"INNER JOIN emc2_teams_2011 AS t2 " .
		"ON t1.team_id = t2.team_id " .
		 "WHERE email = 'tamilsakthi@yahoo.com'; "; //'{$_SESSION['email']}';";
...
//	mysql_free_result($result11);
	...
	//fetch account information
	$query12 =  "SELECT COUNT(*) AS COUNT FROM `emc2_tscores` AS t1 " .
		"INNER JOIN emc2_teams AS t2 " .
		"ON t1.team_id = t2.team_id " .
		 "WHERE email = 'tamilsakthi@yahoo.com';';";
	...
	$team_info_2012 = mysql_fetch_assoc($result12);
	
	$num_rows12 = $team_info_2012["COUNT"];
	// ************************
...
<table style="align:center">
  <tr>
	...
	<input type="text" name="fullname" value="<?php echo htmlspecialchars($ac_info["fullname"]); ?>"  disabled="disabled">
<?php
...
?>
	...
	<input type="text" name="schoolname" value="<?php echo htmlspecialchars($ac_info["schoolname"]); ?>" disabled="disabled">
    <?php
...
<input type="submit" value="Update Account Information"  disabled="disabled" >
</form>
...
?>
<form action="manage.php" method="post">
<input type="hidden" name="team_update" value="true" disabled="disabled" >
<input type="hidden" name="oldname" value="<?php echo $team['name']; ?>" disabled="disabled" >
  ...
  <tr>
    <th>
    	...
    	<input type="text" name="name" value="<?php echo htmlspecialchars($team['name']); ?>" disabled="disabled">
        <?php
...
?>
    ...
    <td>
    	...
    	<input type="text" name="p1" value="<?php echo htmlspecialchars($team['p1']); ?>" disabled="disabled" >
        <?php
...
?>
    ...
    <td>
    	...
    	<input type="text" name="p3" value="<?php echo htmlspecialchars($team['p3']); ?>" disabled="disabled">
        <?php
...
<input type="submit" value="Update Team Information" disabled="disabled" >
</form>
<?php
		}
	}
