...
<h2>Teams registered or paid their fees (FULL TEAMS)</h2>
<table>
  <tr>
    <th>
    	Team ID
...
<?php
		$query = "SELECT team_id, name, email, fee_paid, memo, p1, p2, p3, p4 " . 
			 "FROM emc2_teams where email <> 'a@a.com' and  P1<>'' and p2<>'' and p3<>'' and p4 <>''  order by fee_paid desc,  email asc, team_id asc;";
...
<?php
		$query = "SELECT team_id, name, email, fee_paid, memo, p1, p2, p3, p4 " . 
			 "FROM emc2_teams where email <> 'a@a.com' and ( fee_paid=1 or P1<>'' or p2<>'' or p3<>'' or p4 <>'' or memo <> '') and team_id not in  ( select team_id from emc2_teams where p1<>'' and p2<> '' and p3 <> '' and p4 <> '')  order by fee_paid desc,  email asc, team_id asc;";
		$result = mysql_query($query) 
			or die('Query failed: ' . mysql_error());
		while( $row = mysql_fetch_assoc($result) )
		{
