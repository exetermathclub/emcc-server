...
<?php
		$query = "SELECT fullname, email, schoolname, address, category " .
			 "FROM emc2_accounts where email in (select email from emc2_teams where team_id < 1302 or team_id in (1476, 1477, 1513, 1533, 1536, 1540, 11152 ) ) and category <> 'test' order by SchoolName;";
		$result = mysql_query($query) 
			or die('Query failed: ' . mysql_error());
		while( $row = mysql_fetch_assoc($result) )
		{
