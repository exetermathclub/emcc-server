<?php
     ...
     * Final Project
     *
     * Speed round scoring page
     **********************************************************************/
     
	session_start();

	$pg_name	= "Individual A Scoring";
	...
	else if ($_POST["action"] == "Submit Scores")
	{
		...
		if ($form_valid)
		{
			...
			else if ($oldscores["Asum"] == NULL || $_POST["override"])
			{
				$Asum = 0;
				...
				$submitscores_result = "Successfully added IndivA scores for student $team_id-$indiv_id";
			...
			else
			{
				...
				for ($i = 0; $i <= 20; $i++)
				{
					if ($oldscores["A$i"] != $_POST["A$i"])
					{
						$record_agreement = false;
						$prob_error[$i] = "Original grader said {$oldscores["A$i"]} points";
					}
				}
...
<h1>Individual A Scoring</h1>

<form action="scores-indivA.php" method="post">
...
<?php
	for($i = 1; $i <= 10; $i++)
	{
		$_POST["A$i"] == "3" ? $checkbox_C = 'checked' : $checkbox_C = '';
		$_POST["A$i"] == "0" ? $checkbox_W = 'checked' : $checkbox_W = '';
		...
		<input type="radio" name="A$i" value="3" $checkbox_C >
	    </td> <td>
		<input type="radio" name="A$i" value="0" $checkbox_W >
