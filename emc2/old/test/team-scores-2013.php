<?php
	...
	$query = "SELECT team_id FROM emc2_online_teams WHERE email='$email';";
	...
	//Team Scores
	$tm_query = 
        "SELECT t1.team_id, t2.name, t1.total,  t1.Tsum,  t1.Gsum as guts,  t1.indivsum,  " .
        // "SELECT t1.team_id, t2.name, t1.total, r1.rank as rank, t1.Tsum, r2.rankTSum as rankTeam, t1.Gsum as guts, r3.rankGSum as rankGuts, t1.indivsum, r4.rankIndivSum as rankIndiv, " .
		 " t1.T1, t1.T2, t1.T3, " . 
		 ...
		 " FROM emc2_online_tscores AS t1 " .
		 "INNER JOIN (emc2_online_teams AS t2, emc2_accounts AS t3) ON " .
	...
	//Individual Scores
	$in_query = "SELECT t1.team_id, t4.indiv_id, t4.studentName AS Name, ".
		    "t2.name As 'Team Name', t1.total, t1.ASum as SSum, t1.BSum as ASum,  t1.A1 as S1, t1.A2 as S2, t1.A3 as S3, t1.A4 as S4, " .
		    ...
		    "t1.total FROM emc2_online_iscores AS t1 INNER JOIN " .
		    "(emc2_online_teams AS t2, emc2_accounts AS t3, emc2_online_students " .
		    "AS t4) ON (t1.team_id = t2.team_id AND t2.email = " .
		    "t3.email AND t1.team_id = t4.team_id AND t1.indiv_id " .
		    "= t4.student_key)" . // " inner join " .
