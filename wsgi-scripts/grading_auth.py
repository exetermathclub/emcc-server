...
def application(environ, start_response):
    ...
    c.execute("CREATE TABLE IF NOT EXISTS grading_sesskeys (id INTEGER PRIMARY KEY ASC, sesskey TEXT)")
    ...
    kdict = auth.generateKey(conn, "grading", int(form['A'].value, 16))
    
    # Remember this session key
    c.execute("INSERT INTO grading_sesskeys (sesskey) VALUES (?)", (auth.hexify(kdict["K"]),))
    conn.commit()
    conn.close()
